...
type Props = { yard: YardItem[]; timeOfDay?: number; season?: string }
...
export default function Yard3D({ yard, timeOfDay, season }: Props) {
  ...
  useEffect(() => {
    ...
    const down=(x:number,y:number)=>{isDown=true;lastX=x;lastY=y}
    const move=(x:number,y:number)=>{ if(!isDown)return; theta-=(x-lastX)*0.008; phi-=(y-lastY)*0.006; lastX=x;lastY=y; updateCam() }
    ...
    function frame(){
      ...
      if(!userInteracted && !isDown){ theta+=0.0022; updateCam() }
