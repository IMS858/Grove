...
// SECURITY: host is user-supplied — validate strictly to prevent SSRF.
// Allow only bare hostname/IPv4 with optional port; no scheme, path, creds, or spaces.
const HOST_RE = /^[a-zA-Z0-9]([a-zA-Z0-9.-]{0,251}[a-zA-Z0-9])?(:\d{1,5})?$/
function safeHost(host: string): string | null {
  if (!host || host.length > 260) return null
  if (!HOST_RE.test(host)) return null            // rejects "/", "@", "http://", "?" etc.
  const bare = host.split(":")[0].toLowerCase()
  // Block cloud metadata + loopback probes from our server
...
async function osGet(host: string, path: string) {
  ...
  if (!h) return { ok: false, data: { error: "Invalid controller address" } }
