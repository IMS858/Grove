...
var ALLPLANTS = P.concat(TREES).concat(P2).concat(P3).concat(P4);
...
export default function Grove(){
  ...
  var rStart=function(zoneId,zoneName){
    ...
    irrCall({provider:rConn,action:"start",token:rTok,host:rHost,zoneId:zoneId,duration:rDur*60}).then(function(res){
      setRBusy(false);
      if(res.ok)setRMsg(zoneName+" running for "+rDur+" min");
  ...
  var _zmap=st({}),zMap=_zmap[0],setZMap=_zmap[1]; // {zoneId: [bedId,...]}
  ...
  var _photos=st({}),photos=_photos[0],setPhotos=_photos[1];
  var _photoPlant=st(null),photoPlant=_photoPlant[0],setPhotoPlant=_photoPlant[1];
  var addPhoto=function(plantId,file){var url=URL.createObjectURL(file);setPhotos(function(ps){var np=Object.assign({},ps);var arr=(np[plantId]||[]).slice();arr.unshift({url:url,date:new Date().toISOString().slice(0,10)});np[plantId]=arr;return np})};
  ...
  return <div style={{background:G.bg0,color:G.t1,fontFamily:"-apple-system,Inter,system-ui,sans-serif",minHeight:"100vh",maxWidth:430,margin:"0 auto"}}>
    ...
    {tab==="water"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {rDev?(function(){var pname=rConn==="rachio"?"Rachio":rConn==="netro"?"Netro":"OpenSprinkler";return <>
        ...
        <Card style={{padding:11}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:11,fontWeight:600,color:G.s2}}>{"\u23F1\uFE0F Run duration"}</div><div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={function(){setRDur(Math.max(1,rDur-1))}} style={{background:G.ln,border:"none",borderRadius:8,color:G.t1,width:28,height:28,fontSize:16,cursor:"pointer"}}>-</button><span style={{fontSize:14,fontWeight:700,minWidth:48,textAlign:"center"}}>{rDur+" min"}</span><button onClick={function(){setRDur(Math.min(180,rDur+1))}} style={{background:G.ln,border:"none",borderRadius:8,color:G.t1,width:28,height:28,fontSize:16,cursor:"pointer"}}>+</button></div></div></Card>
        <div style={{fontSize:12,fontWeight:600,marginTop:2}}>Your Zones</div>
        {rDev.zones.map(function(z,zi){var zb=zoneBeds(z.id,zi,rDev.zones.length);return <Card key={z.id} style={{padding:11}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:zb.length?7:0}}>
            <div><div style={{fontSize:12,fontWeight:600}}>{z.name}</div><div style={{color:G.t4,fontSize:9}}>{"Zone "+z.num}</div></div>
            <button onClick={function(){rStart(z.id,z.name)}} disabled={rBusy} style={{background:G.f2,border:"none",borderRadius:9,color:G.s4,fontSize:11,fontWeight:600,padding:"8px 16px",cursor:"pointer",opacity:rBusy?0.5:1}}>{rBusy?"...":"\u{1F4A7} Run "+rDur+"m"}</button>
          </div>
    ...
    {tab==="learn"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
      {gSel?<div style={{display:"flex",flexDirection:"column",gap:9}}>
        ...
        {GUIDES.filter(function(g){return gCat==="All"||g.cat===gCat}).map(function(g){return <Card key={g.id} onClick={function(){setGSel(g)}} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 12px"}}><span style={{fontSize:22}}>{g.e}</span><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600}}>{g.t}</div><div style={{color:G.t4,fontSize:9}}>{g.sub}</div></div><span style={{color:G.t4}}>{"\u203A"}</span></Card>})}
      </>}
    </div>}
