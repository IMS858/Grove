...
// ─── PLANT PHOTOS (fetched from Wikipedia, cached, graceful emoji fallback) ───
var _imgCache = {};
function wikiTerms(name){
...
function PlantImg(p){
  var _u=useState(_imgCache[p.name]===undefined?"loading":_imgCache[p.name]);
  var url=_u[0],setUrl=_u[1];
  useEffect(function(){
    if(_imgCache[p.name]!==undefined){setUrl(_imgCache[p.name]);return}
    var terms=wikiTerms(p.name),i=0,dead=false;
    function tryNext(){
      if(i>=terms.length){_imgCache[p.name]=null;if(!dead)setUrl(null);return}
      var t=terms[i++];
      fetch("https://en.wikipedia.org/api/rest_v1/page/summary/"+encodeURIComponent(t))
        .then(function(r){return r.ok?r.json():null})
        .then(function(d){
          var src=d&&d.type!=="disambiguation"&&d.thumbnail&&d.thumbnail.source;
          if(src){_imgCache[p.name]=src;if(!dead)setUrl(src)}
          else tryNext();
  ...
  var h=p.h||150;
  if(url&&url!=="loading")return <img src={url} alt={p.name} onError={function(){_imgCache[p.name]=null;setUrl(null)}} style={{width:"100%",height:h,objectFit:"cover",borderRadius:14,border:"1px solid "+G.ln,background:G.bg1,display:"block"}}/>;
  return <div style={{width:"100%",height:h,borderRadius:14,border:"1px solid "+G.ln,background:"linear-gradient(135deg,"+G.f1+"33,"+G.bg1+")",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:Math.min(54,h*0.4),opacity:url==="loading"?0.4:0.9}}>{p.e}</span></div>;
}
...
export default function Grove(){
  ...
  var _yard=st(SAVED.yard||[
    // User's real layout: 8 beds in a back row, 6 fruit trees middle-left, front row of beds
    ...
    {id:9,kind:"tree",label:"Eva's Pride",x:6,y:36,w:10,h:10,e:"\u{1F351}"},
    {id:10,kind:"tree",label:"Eva's Pride",x:19,y:39,w:10,h:10,e:"\u{1F351}"},
    {id:11,kind:"tree",label:"Eva's Donut",x:7,y:50,w:10,h:10,e:"\u{1F351}"},
    {id:12,kind:"tree",label:"Golden Apricot",x:20,y:53,w:10,h:10,e:"\u{1F7E0}"},
    {id:13,kind:"tree",label:"Blenheim",x:6,y:64,w:10,h:10,e:"\u{1F7E0}"},
    {id:14,kind:"tree",label:"Dbl Delight",x:19,y:67,w:10,h:10,e:"\u{1F351}"},
    {id:15,kind:"bed",label:"Bed 9",x:38,y:84,w:9,h:5,e:"\u{1F331}"},
    {id:16,kind:"bed",label:"Bed 10",x:49,y:84,w:9,h:5,e:"\u{1F331}"},
    {id:17,kind:"bed",label:"Bed 11",x:60,y:84,w:9,h:5,e:"\u{1F331}"},
    {id:18,kind:"bed",label:"Bed 12",x:71,y:84,w:9,h:5,e:"\u{1F331}"},
    {id:19,kind:"bed",label:"Bed 13",x:82,y:84,w:9,h:5,e:"\u{1F331}"},
    {id:20,kind:"bed",label:"Bed 14",x:38,y:74,w:9,h:5,e:"\u{1F331}"},
  ...
  var _hasUser=st(true),hasUser=_hasUser[0],setHasUser=_hasUser[1]; // signed in? (assume yes until checked)
  ...
  return <div style={{background:G.bg0,color:G.t1,fontFamily:"-apple-system,Inter,system-ui,sans-serif",minHeight:"100vh",maxWidth:wide?"none":430,margin:"0 auto",paddingLeft:wide?210:0}}>
    <style dangerouslySetInnerHTML={{__html:
      ...
      "input:focus,select:focus{outline:none;border-color:"+G.s2+" !important;box-shadow:0 0 0 3px "+G.s2+"22}"+
      "::selection{background:"+G.f2+";color:"+G.s4+"}"+
      "::-webkit-scrollbar{width:8px;height:8px}::-webkit-scrollbar-thumb{background:"+G.ln+";border-radius:99px}::-webkit-scrollbar-track{background:transparent}"
    }}/>
    <div style={{padding:wide?"16px 32px 14px":"11px 16px 9px",display:"flex",justifyContent:wide?"flex-end":"space-between",alignItems:"center",borderBottom:"1px solid "+G.ln,maxWidth:wide?1200:"none",margin:wide?"0 auto":0}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>{hydrated&&!hasUser&&<button onClick={function(){window.location.href="/login"}} style={{background:G.f2+"22",border:"1px solid "+G.f2+"44",borderRadius:99,color:G.s3,fontSize:9,fontWeight:600,padding:"3px 9px",cursor:"pointer"}}>{"\u2601 Sign in to sync"}</button>}{(syncState==="saved"||syncState==="saving"||syncState==="offline")&&<span title={syncState==="offline"?"Saved on this device":"Synced to cloud"} style={{fontSize:10,color:syncState==="offline"?G.t4:G.s2}}>{syncState==="saving"?"\u27F3":syncState==="offline"?"\u25CB":"\u2601"}</span>}<span style={{fontSize:9,color:G.s2,fontWeight:600,border:"1px solid "+G.s2+"44",borderRadius:99,padding:"2px 8px"}}>9B</span>{wx&&wx.current.t!==null&&<><span style={{fontSize:13}}>{wx.current.i}</span><span style={{fontSize:12,fontWeight:600}}>{wx.current.t+"\u00B0"}</span></>}</div>
    </div>
    ...
    {tab==="home"&&<div style={{display:"flex",flexDirection:"column",gap:12,maxWidth:wide?620:"none",margin:wide?"0 auto":0,width:"100%"}}>
      {/* ── Hero: date + greeting + season + weather in one card ── */}
      {(function(){var s=seasonNow();return <div style={{position:"relative",overflow:"hidden",borderRadius:18,border:"1px solid "+G.f2+"33",background:"linear-gradient(135deg,"+G.f1+"3A 0%,"+G.bg1+" 55%,"+G.bg1+" 100%)",padding:"16px 16px 14px"}}>
        <div style={{position:"absolute",top:-30,right:-30,width:130,height:130,borderRadius:"50%",background:"radial-gradient(circle,"+G.f2+"33,transparent 70%)"}}/>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",position:"relative"}}>
          <div>
            <div style={{color:G.s2,fontSize:10,fontWeight:600,letterSpacing:".5px",textTransform:"uppercase"}}>{todayStr()}</div>
            <div style={{color:G.t1,fontSize:24,fontWeight:800,marginTop:2,letterSpacing:"-.6px"}}>{greeting()+"."}</div>
          </div>
          <span style={{fontSize:30,lineHeight:1}}>{s.e}</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:7,marginTop:9,position:"relative"}}>
          <span style={{background:G.f2+"26",border:"1px solid "+G.f2+"44",borderRadius:99,padding:"3px 10px",color:G.s3,fontSize:10,fontWeight:700}}>{s.n+" \u00B7 9b"}</span>
          {wx&&wx.current.t!==null&&<span style={{background:G.bg0+"66",border:"1px solid "+G.ln,borderRadius:99,padding:"3px 10px",color:G.t2,fontSize:10,fontWeight:600}}>{wx.current.i+" "+wx.current.t+"\u00B0"}</span>}
        </div>
        <div style={{color:G.t3,fontSize:10,lineHeight:1.5,marginTop:8,position:"relative"}}>{s.t}</div>
      </div>})()}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div onClick={function(){setCalMode("plants");setTab("calendar")}} style={{cursor:"pointer"}}><div style={{color:G.c2,fontSize:20,fontWeight:700}}>{myPlants.length}</div><div style={{color:G.t4,fontSize:9}}>TRACKED</div></div>
              <div onClick={function(){setTab("garden")}} style={{cursor:"pointer"}}><div style={{color:G.s3,fontSize:20,fontWeight:700}}>{beds.length}</div><div style={{color:G.t4,fontSize:9}}>BEDS</div></div>
              <div onClick={function(){setTab("garden")}} style={{cursor:"pointer"}}><div style={{color:G.f3,fontSize:20,fontWeight:700}}>{orchard.length}</div><div style={{color:G.t4,fontSize:9}}>TREES</div></div>
              <div style={{cursor:"default"}}><div style={{color:G.s2,fontSize:20,fontWeight:700}}>{tasks.filter(function(t){return!t.d}).length}</div><div style={{color:G.t4,fontSize:9}}>TO DO</div></div>
            </div>
      ...
      <div style={{display:"flex",gap:6}}>{[{l:"Water",i:"\u{1F4A7}",go:function(){setTab("water")}},{l:"Feed",i:"\u{1F33F}",go:function(){setFdMode("schedule");setTab("learn")}},{l:"pH",i:"\u{1F9EA}",go:function(){setTab("garden")}},{l:"Harvest",i:"\u{1F345}",go:function(){setCalMode("plants");setTab("calendar")}}].map(function(b,i){return <Card key={i} onClick={b.go} style={{flex:1,textAlign:"center",padding:"8px 4px",borderColor:G.f2+"33",cursor:"pointer"}}><div style={{fontSize:16}}>{b.i}</div><div style={{color:G.t3,fontSize:9,marginTop:2}}>{b.l}</div></Card>})}</div>
    ...
    {tab==="yard"&&(function(){
        // ── 3D YARD DESIGNER (full editor) ──
        var palette=[{kind:"bed",label:"Bed",e:"\u{1F331}",w:9,h:5},{kind:"plant",label:"Plant",e:"\u{1F33F}",w:4,h:4},{kind:"tree",label:"Tree",e:"\u{1F333}",w:11,h:11},{kind:"citrus",label:"Citrus",e:"\u{1F34A}",w:10,h:10},{kind:"container",label:"Pot",e:"\u{1FAB4}",w:6,h:6},{kind:"structure",label:"Shed",e:"\u{1F3DA}\uFE0F",w:14,h:10},{kind:"compost",label:"Compost",e:"\u267B\uFE0F",w:7,h:7},{kind:"water",label:"Water",e:"\u{1F6B0}",w:6,h:6},{kind:"path",label:"Path",e:"\u{1F7EB}",w:30,h:6}];
        var sel=yard.find(function(i){return i.id===yardSel});
        return <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:14,fontWeight:600}}>{"\u{1F5FA}\uFE0F Your Yard \u2728"}</div>
            <span style={{color:G.t4,fontSize:9}}>{yard.length+" pieces"}</span>
          </div>
          <div style={{color:G.t3,fontSize:10,lineHeight:1.5}}>{"Tap anything to select it \u00B7 drag it to move \u00B7 drag empty ground to orbit \u00B7 pinch or scroll to zoom."}</div>
          <Yard3D yard={yard} timeOfDay={y3time} season={y3season} selectedId={yardSel}
            onSelect={function(id){setYardSel(id)}}
            onMove={function(id,nx,ny){setYard(function(ys){return ys.map(function(it){return it.id===id?Object.assign({},it,{x:nx,y:ny}):it})})}}/>
          ...
          <div style={{color:G.t4,fontSize:9,lineHeight:1.4}}>Everything saves automatically and drives your bed & orchard views.</div>
        </div>;
    ...
    {tab==="library"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
      {sel?<div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        <PlantImg name={sel.n} e={sel.e} h={160}/>
    ...
    {tab==="learn"&&<div style={{display:"flex",flexDirection:"column",gap:10,maxWidth:wide?820:"none",margin:wide?"0 auto":0,width:"100%"}}>
