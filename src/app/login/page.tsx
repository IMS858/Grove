...
export default function Login() {
  ...
  const btn: React.CSSProperties = { background: G.s3, color: G.bg0, border: "none", borderRadius: 10, padding: "13px 0", width: "100%", fontSize: 14, fontWeight: 700, cursor: "pointer" }
  ...
  return (
    <div style={{ background: G.bg0, color: G.t1, fontFamily: "-apple-system,Inter,system-ui,sans-serif", minHeight: "100vh", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        ...
        <p style={{ color: G.t3, fontSize: 13, marginTop: 4 }}>Log in to your garden</p>
      </div>

      <button onClick={signInGoogle} style={{ background: "#fff", color: "#1a1a1a", border: "none", borderRadius: 10, padding: "12px 0", width: "100%", fontSize: 14, fontWeight: 600, cursor: "pointer", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      {mode === "password" && <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} style={inp} />}

      <button onClick={mode === "password" ? signInPassword : signInMagic} disabled={loading} style={{ ...btn, opacity: loading ? 0.6 : 1 }}>
        {loading ? "..." : mode === "password" ? "Log In" : "Send Magic Link"}
      </button>

      <button onClick={() => { setMode(mode === "password" ? "magic" : "password"); setMsg("") }} style={{ background: "none", border: "none", color: G.t3, fontSize: 12, cursor: "pointer", marginTop: 12 }}>
      ...
      <div style={{ textAlign: "center", marginTop: 24 }}>
        <span style={{ color: G.t3, fontSize: 13 }}>No account? </span>
        <button onClick={() => router.push("/signup")} style={{ background: "none", border: "none", color: G.s3, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Sign up</button>
      </div>
    </div>
  )
}
